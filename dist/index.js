...
var Nav = (function () {
    function Nav(element) {
        ...
        this.rootItems = element.querySelector('.silk-nav--items');
        // Set initial position
        this.position = 0;
        // For each nav items
        [].forEach.call(this.rootItems.querySelectorAll('.silk-nav--items'), function (items) {
            // Get elements
            var item = items.parentNode;
            var link = item.querySelector('.silk-nav--link');
            // Get link text
        ...
        [].forEach.call(element.querySelectorAll('.silk-nav--move-forward'), function (moreLink) {
        ...
        [].forEach.call(element.querySelectorAll('.silk-nav--move-back'), function (backLink) {
    ...
    Nav.prototype.move = function (event) {
        event.preventDefault();
        // Get parent item
        var parentItem = event.target.parentNode.parentNode;
        // Hide everything
        [].forEach.call(this.rootItems.querySelectorAll('.silk-nav--items'), function (el) {
            el.classList.add('is-hidden');
        });
        // Show selected branch
        [].forEach.call(parentItem.querySelectorAll('.silk-nav--items'), function (el) {
            el.classList.remove('is-hidden');
        });
        // Show selected branch parent tree
        [].forEach.call(this.getParents(event.target, '.silk-nav--items'), function (el) {
            el.classList.remove('is-hidden');
        });
        // Get parent items
        var parentItems = parentItem.querySelector('.silk-nav--items');
        // Add CSS for move
        this.rootItems.style.left = (this.position * -100) + '%';
        // Add CSS for height
        this.rootItems.style.height = parentItems.offsetHeight + 'px';
    };
    Nav.prototype.getParents = function (elem, selector) {
